...
class DependencyAnalyzer:
    ...
    def analyze_dependencies(self) -> List[str]:
        ...
        if has_python_files or has_requirements_txt:
            ...
            requirements_deps = self._get_requirements_dependencies(agent_library_dir)
            deps.extend(requirements_deps)
            
            # Then, scan Python files for actual imports
            import_deps = self._scan_python_imports(agent_library_dir)
    ...
    def _get_requirements_dependencies(self, agent_library_dir: Path) -> List[str]:
        """Extract dependencies from requirements.txt files"""
        deps = []
        requirements_paths = [
            agent_library_dir / "requirements.txt",
    ...
    def _scan_python_imports(self, agent_library_dir: Path) -> List[str]:
        ...
        try:
            ...
            python_files = list(agent_library_dir.glob("**/*.py"))
